...
function App() {
  ...
  const [cloakSchoology, setCloakSchoology] = useState(
    localStorage.getItem('cloak') === 'schoology'
  );
  ...
  useEffect(() => {
    const updateFlag = () => setCloakSchoology(localStorage.getItem('cloak') === 'schoology');
    updateFlag();
  ...
  useEffect(() => {
    const setCloakStuff = () => {
      ...
      let title = "Insanity";
      let href = "/insanity-logo.png";
      if (cloak === "schoology") {
        title = "Login to Schoology";
        href = "/schoology.png";
      } else if (cloak === "google") {
        title = "Google";
        href = "/google.png";
      } else if (cloak === "powerschool") {
        title = "Student and Parent Sign In";
        href = "/powerschool.ico";
      }
    ...
    setCloakStuff();
    window.addEventListener("cloak-changed", setCloakStuff);
    return () => window.removeEventListener("cloak-changed", setCloakStuff);
  }, [location]);
  ...
  <ul className="hidden md:flex gap-8 text-base font-medium">
    ...
    <li>
      <Link to="/settings" className="hover:text-[var(--wope-purple)] cursor-pointer">Settings</Link>
    </li>
  </ul>
            ...
            <>
              <header className="flex flex-col items-center justify-center text-center px-4 md:px-0 mt-12 mb-10 relative">
                <img
                  ...
                  alt="hero-bg"
                  className="absolute z-0 w-full left-0 top-0 max-h-[600px] object-cover opacity-60"
                />
                <img
                  src="/images/hero-lights.png"
                  alt="hero-lights"
                  className="absolute z-10 left-1/2 top-0 transform -translate-x-1/2 w-[900px] h-[650px] pointer-events-none select-none"
                />
                ...
                <img
                  ...
                  alt="hero-top"
                  className="absolute left-0 top-0 -z-10 w-full pointer-events-none select-none"
                />
                <img
                  src="/images/hero-bottom.png"
                  alt="hero-bottom"
                  className="absolute left-0 bottom-0 -z-10 w-full pointer-events-none select-none"
                />
              </header>
              <section className="relative z-10 w-full flex flex-col items-center mt-16 px-4 md:px-0">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
                  <div className="group bg-gradient-to-br from-[var(--wope-purple)]/70 to-[var(--wope-blue)]/80 rounded-2xl p-7 shadow-xl hover:scale-[1.03] hover:shadow-2xl transition overflow-hidden cursor-pointer relative">
                    <img
                      src="/images/keyword-tag.png"
                      alt="Auto Keyword Tagging"
                      className="absolute -top-8 -right-8 w-28 opacity-30 group-hover:opacity-60 transition"
                    />
                    <div className="flex items-center gap-3 mb-4">
                      <img src="/images/student.png" alt="Student Icon" className="w-9 h-9 object-contain" />
                      <span className="font-head text-xl">Anonymous Student</span>
                    </div>
                    <p className="text-sm md:text-base text-white/80 pb-2 font-medium">
                      Insanity is really cool! It's an unblocked game site where I can play games at school, and it's super easy to use. I told my friends about it, and now we all play together during breaks. It's made school a lot more fun and helped me meet more people.
                    </p>
                  </div>
                  <div className="group bg-gradient-to-br from-[var(--wope-bg)]/80 to-[var(--wope-lavender)]/80 rounded-2xl p-7 shadow-xl hover:scale-[1.03] hover:shadow-2xl transition overflow-hidden cursor-pointer relative">
                    <img
                      ...
                      alt="Recommendation"
                      className="absolute -top-8 right-0 w-28 opacity-20 group-hover:opacity-50 transition"
                    />
                    <div className="flex items-center gap-3 mb-4">
                      <img src="/images/employee.png" alt="Employee Icon" className="w-9 h-9 object-contain" />
                      <span className="font-head text-xl">Exhausted Employee</span>
                    </div>
                    <p className="text-sm md:text-base text-white/80 pb-2 font-medium">
                      After long days at work, I've found Insanity to be a great way to unwind. Since it's unblocked, I can easily access it during my breaks. Playing games helps me recharge a bit, and I've even shared it with my colleagues. It's become a nice escape that helps ease the exhaustion of a busy workweek.
                    </p>
                  </div>
                  <div className="group bg-gradient-to-br from-[var(--wope-dark-purple)]/90 to-[var(--wope-purple)]/80 rounded-2xl p-7 shadow-xl hover:scale-[1.03] hover:shadow-2xl transition overflow-hidden cursor-pointer relative">
                    <img
                      src="/images/traffic-estimation.png"
                      alt="Traffic Estimation"
                      className="absolute -top-8 -left-8 w-32 opacity-20 group-hover:opacity-50 transition"
                    />
                    <div className="flex items-center gap-3 mb-4">
                      <img src="/images/website-owner.png" alt="Website Owner Icon" className="w-9 h-9 object-contain" />
                      <span className="font-head text-xl">Fellow Website Owner</span>
                    </div>
                    <p className="text-sm md:text-base text-white/80 pb-2 font-medium">
                      As a fellow site owner, I truly admire Insanity. My site is dynamic, but Insanity excels as a static website. It's well-designed and offers a fantastic user experience. It's impressive to see how a simpler format can still deliver so much value!
                    </p>
                  </div>
                </div>
              </section>
                ...
                <div className="order-2 md:order-1 flex flex-col justify-center items-start md:items-start">
                  ...
                  <ul className="flex flex-col gap-2 mb-6 w-full">
                    <li className="flex items-center gap-2">
                      <span className="inline-block w-2 h-2 bg-[var(--wope-purple)] rounded-full" /> 1-click instant play
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="inline-block w-2 h-2 bg-[var(--wope-blue)] rounded-full" /> 100% unblocked and safe
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="inline-block w-2 h-2 bg-[var(--wope-lavender)] rounded-full" /> Works at school and work
                    </li>
                  </ul>
                  <Link
                    to="/games"
              ...
              <footer className="w-full bg-[var(--wope-nav-bg)] px-8 pt-14 pb-7 text-white flex flex-col items-center">
                <div className="flex flex-col md:flex-row justify-between items-center max-w-6xl w-full gap-10 md:gap-4">
                  ...
                  <nav className="flex space-x-8">
                    <Link to="/games" className="hover:text-[var(--wope-purple)] transition">
                      Games
                    </Link>
                    <Link to="/ai" className="hover:text-[var(--wope-purple)] transition">
                      AI
                    </Link>
                    <Link to="/browse-unrestricted" className="hover:text-[var(--wope-purple)] transition">
                      Proxy
                    </Link>
                    <Link to="/settings" className="hover:text-[var(--wope-purple)] transition">
                      Settings
                    </Link>
                  </nav>
                  <div className="flex space-x-5 mt-5 md:mt-0">
                    <a
                      href="https://discord.com/invite/Gv4rVt2SJp"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-[var(--wope-purple)] transition"
                      aria-label="Discord"
                    >
                    ...
                    <a
                      href="https://github.com/321EZ123/Insanity-V1-Release"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-[var(--wope-purple)] transition"
                      aria-label="GitHub"
